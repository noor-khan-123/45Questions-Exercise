...
const Admin = ['admin', 'eric', 'sara', 'bob', 'jane'];
...
const current_users = ['alice', 'bob', 'charlie', 'david', 'echo'];
const new_users = ['daniel', 'frank', 'grace', 'david', 'ian'];

// Loop through the new_users list to see if each new username has already been used
for (let i = 0; i < new_users.length; i++) {
  const new_username = new_users[i].toLowerCase(); // Convert new_username to lowercase

  // Check if the new_username is already in the current_users list
  let is_unique = true; // Assume the new_username is unique until proven otherwise
  ...
  if (is_unique) {
    ...
    console.log(`${new_username} is not available. Please enter a new username.`);
  }
}
...
const ordinal_suffix = ['th', 'st', 'nd', 'rd', 'th', 'th', 'th', 'th', 'th'];
  
for (let i = 0; i < numbers.length; i++) {
    const num = numbers[i];
    const suffix = num <= 3 ? ordinal_suffix[num] : ordinal_suffix[0];
