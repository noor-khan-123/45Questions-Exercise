...
function pizzas() {
    const pizzas = ['pepperoni', 'cheese', 'veggie'];
    const like_statement = 'I like ';
      
    for (let i = 0; i < pizzas.length; i++) {
          console.log(`${like_statement}${pizzas[i]} pizza.`);
    }
...
const common_trait = 'would make a great pet';
      
for (let i = 0; i < animals.length; i++) {
          console.log(`A ${animals[i]} ${common_trait}.`);
 }
      
    console.log(`Any of these animals ${common_trait}!`);
}
...
function Tshirt(size, message) {
        console.log(`I'm going to make a ${size} t-shirt with the message: ${message}`);
}
